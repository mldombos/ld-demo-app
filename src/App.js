...
function App() {
  ...
  return (
    <div className="App h-screen bg-ldls grid xl:grid-cols-4 grid-cols-3 grid-rows-4 xl:grid-rows-3 bg-cover bg-no-repeat">
      ...
      {qrcode ? (
        <div className="body grid row-start-1 col-span-3 xl:col-span-1 xl:col-start-1 xl:row-start-1 bg-black-4 place-items-center">
            <QRCode /> 
        </div>
      ) : null}
      {login ? (
        <div className="body grid col-span-3 row-start-4 xl:col-span-1 xl:col-start-1 xl:row-start-2 bg-black-4 place-items-center">
          <Login />
        </div>
      ) : null}
      {upperimage ? (
        <div className="body grid row-start-1 col-span-3 xl:col-start-2  xl:row-start-1 place-items-center">
          {logoversion ? (
            <img
              src={toggle}
